...
- 成就系统：奖杯入口显示“👍”“完美通关”“我爱arrow”“有点牛逼”四项成就；第四项在单次无尽挑战通过 10 关后解锁，并以牛脸图标展示。

## Windows 运行

完整游戏可直接运行 `dist/ArrowOrder.exe`，不需要安装 Python 或联网。v1.0.4 使用全新的 `progress_v2.json` 存档名，不读取旧版 `progress_v1.json`，因此首次启动时普通关卡、无尽记录和四项成就都是未完成状态；之后的新进度会在用户的 APPDATA 目录保存。发布 EXE 时请同时保留字体许可文件 `assets/fonts/OFL.txt`。
...
无尽模式的棋盘大小为 5×5 至 7×7，箭头数量和阻挡层数参考普通关卡的难度范围。游戏页的“保存本局”会保存当前棋盘、轮次、剩余机会、提示次数和用时；返回首页或关闭窗口时也会自动保存。首页出现“继续无尽”后可以恢复上次进度，也可以选择“新开无尽”放弃旧局重新开始。每局重新开始时恢复 3 次机会和 3 次提示；程序分别记录累计通关数与单次挑战最高通关数，不影响普通关卡存档。单次通过 10 关后会解锁“有点牛逼”成就。
