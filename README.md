...
操作说明：左键点击箭头；点击“自动求解”会按当前关卡的合法顺序逐步清空棋盘，按钮随后变为“停止自动求解”，再次点击会在当前箭头完成后恢复手动操作；提示高亮状态下也可以直接启动；飞出反馈约 0.18 秒、碰撞反馈约 0.14 秒，方便连续操作；按 `R` 或点击重开按钮重新开始；提示按钮显示剩余提示；`Esc` 退出。自动求解期间棋盘点击会被忽略，但可以停止、重开或返回。通关后可以进入下一关或打开选关页面。
